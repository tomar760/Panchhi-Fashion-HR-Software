...
async function syncToCloud(payload, btn) {
    ...
    try {
        ...
        setTimeout(() => { btn.innerHTML = originalText; btn.classList.remove("opacity-50", "cursor-not-allowed"); btn.classList.replace("from-emerald-500", "from-indigo-600"); btn.disabled = false; }, 3000);
        if (result.status === "success") return true;
        else { alert("❌ Error: " + result.message); return false; }
    } catch (error) { btn.innerHTML = originalText; btn.disabled = false; alert("❌ Network Error!"); return false; }
}
